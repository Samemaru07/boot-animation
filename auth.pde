...
void keyPressed() {
  if (phase==3 && pwVisible && !authDone && !timedOut) {
    if (key==ENTER || key==RETURN) {
      authDone=true; pwVisible=false;
      if (pwInput.length()>0) doSuccess(); else doFail("NULL INPUT");
...
void doSuccess() {
  ...
  pcPower="TRANSFERRING"; pcPowerCol=color(68,255,136);
  progColor=COL_OK; phase=4;
}
...
void doFail(String reason) {
  ...
  flickering=true; flickerCount=0; flickerLast=millis();
}

void onFlashDone() {
  if (pwAttempts<3) {
    addLog("[WARN] RETRY "+pwAttempts+"/3", COL_MAIN);
    pwInput=""; authDone=false; timedOut=false;
    pwVisible=true; authStart=millis();
    ...
    progWidth=100; progColor=COL_ERR;
  }
}
