...
float[] shieldRadius    = {0, 0, 0};     // 各レイヤーの現在半径
float[] shieldTargetR   = {170, 210, 250};  // 目標半径
float[] shieldAlpha     = {0, 0, 0};
float   shieldGlow      = 0;
...
void drawWelleShield() {
  ...
  float p = phaseProgress(3.0);  // Phase 2の3秒間で展開
  ...
  for (int i = 0; i < 3; i++) {
    ...
    float layerP = constrain((p - layerDelay / 3.0) / (1.0 - layerDelay / 3.0), 0, 1);
    ...
    for (int g = 4; g >= 1; g--) {
      ...
      strokeWeight(gr);
      ellipse(0, 0, rCurrent * 2 + g * 3, rCurrent * 2 + g * 3);
    }

    // リング本体
    stroke(68, 200, 255, 200 * a);
    strokeWeight(2.0);
    ellipse(0, 0, rCurrent * 2, rCurrent * 2);

    // 内側の薄いリング
    stroke(68, 170, 255, 80 * a);
    strokeWeight(1);
    ellipse(0, 0, rCurrent * 2 - 6, rCurrent * 2 - 6);
    ...
    if (layerP < 0.6) {
      ...
      strokeWeight(1.5);
      noFill();
      ellipse(0, 0, rippleR * 2, rippleR * 2);
    }
    ...
    for (int m = 0; m < marks; m++) {
      float angle = TWO_PI / marks * m + gTime * 0.08 * (i % 2 == 0 ? 1 : -1);
  ...
  if (ePhase == 2 && phaseProgress(3.0) > 0.9) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      strokeWeight(8);
      noFill();
      ellipse(0, 0, shieldRadius[i] * 2, shieldRadius[i] * 2);
    }
    // WELLE SHIELD ラベル
