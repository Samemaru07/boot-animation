...
void drawBlackout() {
  ...
  if (ePhase == 3 && p < 0.5) {
    float flashP = p / 0.5;
    // 複数回フラッシュ
    float flashSin = sin(flashP * PI * 5);
    redFlashAlpha = max(0, flashSin) * 80;

    pushStyle();
    noStroke();
    fill(255, 20, 20, redFlashAlpha);
  ...
  // ─── ブラックアウト（Phase 3 後半〜） ──────────────────
  if (ePhase == 3 && p > 0.45) {
    float fadeP = map(p, 0.45, 1.0, 0, 1.0);
    blackoutAlpha = easeIn(fadeP) * 255;
  } else if (ePhase >= 4) {
    blackoutAlpha = 255;
  }
